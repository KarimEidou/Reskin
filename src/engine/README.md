...
commits a transform or closes a lasso polygon, Escape cancels, Backspace or
Delete removes the last polygon corner, arrows nudge; it returns false for
keys the tool did not use, so the UI's own shortcuts can run) and modifier changes
