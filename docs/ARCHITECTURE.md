...
* `windows/{box_window,editor_window}.rs` build windows `from_config` + WebView2
  tuning; `windows/mailbox.rs` (seq queue + long-poll, generations);
  `windows/morph.rs` (handoff FSM with acks/timeouts/fallback, the box at
  rest); `windows/rules.rs` (its decisions, unit tested: who hands over,
  when the box may show, what the hotkey does); `windows/animator.rs` (box
  motion, drag loop, fling/snap, flights).
